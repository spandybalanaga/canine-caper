...
pothos = pg.image.load("bar plant.png")


cMonsterra = False #clicked Monsterra
pMonsterra = False #planted monsterra
ptMonsterra = False #planted monsterra text box
monsterra = pg.image.load("street plant.png")


##---------------------STUFF-----------------------------------
peanutbutter = pg.image.load("peanutbutterjar.png")
flour = pg.image.load("flourbag.png")
...
crystal = pg.image.load("crystal.png")

## -----------------PEOPLE-------------------------------

baker = pg.image.load("baker_temp.png")
bartender = pg.image.load("bartender_temp.png")
cashier = pg.image.load("cashier_temp.png")
dj = pg.image.load("dj_temp.png")
psychic = pg.image.load("fortuneteller_temp.png")
gardener = pg.image.load("gardener_temp.png")
...
def danceSeq():
    ...
    dancemoves = [pg.image.load("dj dance1.png"), pg.image.load("dj dance1.png"),pg.image.load("dj dance1.png"),
                  pg.image.load("dj dance2.png"), pg.image.load("dj dance2.png"),
                  pg.image.load("dj dance3.png"),pg.image.load("dj dance3.png"), pg.image.load("dj dance3.png"),
                  pg.image.load("dj dance2.png"),pg.image.load("dj dance2.png")]
    ...
    if danceTimer < 6:
        ...
        if currentTime >= endTime:
            finDance = False
        return pg.image.load("dj_temp.png")
...
def textbox(string1, string2):
    ...
    words1 = font.render(str(string1), 1, [0,0,0])
    words2 = font.render(str(string2), 1, [0,0,0])
...
def introSeq():
    ...
    words = font.render("A dog just stole your bus pass !!! how are you going to get home ????", 1, [255,255,255])
    words2 = font.render("FIND THAT DOG", 1, [255,255,255])
    words3 = font.render("(P.S dont worry im gonna make this pretty and animate it just not yet)", 1, [255,255,255])
    words4 = font.render("PRESS SPACE TO CONTINUE", 1, [255,255,255])
    screen.blit(words, (200,50))
    screen.blit(words2, (400,100))
    screen.blit(words3, (200,170))
    screen.blit(words4, (400,300))
...
##-----------------MVMT and CLICK INPUTS----------------
            ...
            if event.type == pg.MOUSEBUTTONUP: ##if you click
                ...
                if posy > 300 and posy < 360 and posx > 510 and posx < 900 :
                    if pothos in inventory:
                        ...
                        pothos = pg.image.load("pothos planted.png")
                        
                    if monsterra in inventory:
                        pMonsterra = True
                        ptMonsterra = True
                        inventory.remove(monsterra)
                        monsterra = pg.image.load("monstera planted.png")
        ...
        if rooms[currentRoom] == street: #While in the Street
            if event.type == pg.MOUSEBUTTONUP: ##if you click
                ...
                if psychic.get_rect(topleft = (600,70)).collidepoint(pos):
                    endTime = currentTime + 5000
                    cPsychic = True
...
##------------------------------------DISPLAY NPCs-----------------------------------------------------
    ...
    if rooms[currentRoom] == street:
        rooms[currentRoom].screen.blit(psychic,(600,70))
...
##--------------------------------TEXT BOXES----------------------------------# text box needs to be on top of player 
    ...
    if rooms[currentRoom] == street:
        if cMonsterra == True and cPsyIB == False: 
        ...
        if cPsychic == True:
            if not ("tea leaves" or "crystal ball" or "bake cookies") in missions:
                textbox("A dog stole your bus pass?? And you want to get it back?", "I could ask the spirit world for answers if you want!")
                if currentTime > endTime:
                    cPsyIB = True
                    cPsychic = False
